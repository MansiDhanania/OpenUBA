...
def shapplotsvc(data):
  ...
  data_copy = data.copy().sample(200, random_state=200)
  ...
  # Sample training data for performance (5K instances for SVC due to O(n²) complexity)
  import numpy as np
  if len(X_train) > 5000:
  ...
  explainer = shap.Explainer(svc.predict, X_test_sample, max_evals=1000)
